...
public class AsyncGetService {
    ...
    @Async
    public <T> Future<Resources<T>> getResourceList(URI url, TypeReferences.ResourcesType<T> type) throws
        InterruptedException, IOException {
        RequestEntity<Void> request = RequestEntity.get(url).accept(HAL_JSON).build();
        logger.debug("Requesting: " + request.toString());
    ...
    @Async
    public <T> Future<List<T>> getResourceList(URI url, ParameterizedTypeReference<List<T>> type) throws
        InterruptedException, IOException {
