...
public class OrdersController {
    ...
    public @ResponseBody CustomerOrder newOrder(@RequestBody NewOrderResource item) {
        try {
            ...
            logger.debug("Starting calls");
            
            Future<Resource<Address>> addressFuture = asyncGetService.getResource(
                item.address, new TypeReferences.ResourceType<Address>() {});
            ...
            Future<Resource<Card>> cardFuture = asyncGetService.getResource(
                item.card, new TypeReferences.ResourceType<Card>() {});
            Future<List<Item>> itemsFuture = asyncGetService.getResourceList(
                item.items, new ParameterizedTypeReference<List<Item>>() {});
            
            logger.debug("End of calls.");
            ...
            Future<Shipment> shipmentFuture = asyncGetService.postResource(
                config.getShippingUri(), new Shipment(customerId), new ParameterizedTypeReference<Shipment>() {});
            ...
            logger.debug("Received data: " + order.toString());
